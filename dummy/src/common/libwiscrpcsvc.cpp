...
ssize_t timeout_recv(int fd, void *buf, size_t count, int timeout_seconds=15) {
	fd_set fds;
	size_t bytesread = 0;
...
ssize_t timeout_send(int fd, const void *buf, size_t count, int timeout_seconds=5) {
	fd_set fds;
...
void RPCSvc::connect(std::string host, uint16_t port) {
	...
	if (flags != -1) {
        ...
        close(this->fd);
        freeaddrinfo(resolution);
        throw ConnectionFailedException(std::string("Unable to set socket to non-blocking mode: ") + stdstrerror(errno));
    	}
	} 
	else {
    close(this->fd);
    ...
    if (errno != EINPROGRESS) {
		...
		if (timeout_recv(this->fd, &protover, 4) != 4) {
			printf("timeout in  connection module \n");
			int err = errno;
			close(this->fd);
			this->fd = -1;
			if (errno)
		...
		printf("Success in  connection module \n");
...
RPCMsg RPCSvc::call_method(const RPCMsg &reqmsg)
{
	...
	if (timeout_recv(this->fd, &rsplen, 4) != 4) { // Allow a 5 minute timeout for the response execution and first return, rather than 30s for message transfer.
		printf("timeout in  call method module \n");
		close(this->fd);
		this->fd = -1;
